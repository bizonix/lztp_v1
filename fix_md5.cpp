...
void md5_process_block (void *buffer, size_t len, struct md5_ctx *ctx);
void md5_process_bytes (void *buffer, size_t len, struct md5_ctx *ctx);

byte fillbuf[64] = { 0x80, 0   };
...
int main( int  argc, char **argv ) {
  ...
  byte md5buffer[16];
  char* file = argv[1];
  int fail = md5_file( file, 1, md5buffer );
  for( i=0; i<16; ++i ) printf( "%02x", md5buffer[i] );
  printf( "\n" );
